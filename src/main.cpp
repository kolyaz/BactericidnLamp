...
int8_t CiclZnach (int8_t UpPredel, int8_t DnPredel, int16_t rotateCZ)       //Функция чтения циклического значения в определенных пределах
{
           // printf("rotateCZ %i\n", rotateCZ);
           // printf("UpPredel %i\n", UpPredel);
...
bool TikTime (int TimePrOld, int TimePr)                                    //Функция для формирования импульса для различных промежутком времени
 {
        //printf("rotateCZ %i\n", TimePrOld);
        //printf("UpPredel %i\n", TimePr);
        if (TimePrOld != TimePr)
...
void TimerObsh (int TimerSec,Timer &t)              //---Счетчик времени
{
...
void drebezg_encoder (int dredezg)              //исключение дребезка энкодера. включение/выключение подсветки экрана
...
{   
    ...
    if (LightDisp and button and (Level == 0))  //Условие, при котором, при нажатии на кнопку будет переход на уровень 1
    {
        ...
        else if (button and (Level == 1) and !Level1Back)   //Условие пререхода на второй уровень (Начальнй ресурс ламп, сброс остаточного ресурса к начальному, возврат на предыдущее меню)
    {
        //lcd.cls();
